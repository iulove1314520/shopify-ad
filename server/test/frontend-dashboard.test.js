...
test('平台状态会展示 TikTok Purchase 模式，帮助确认当前单一来源策略', async () => {
  const { app } = loadDashboardApp({
    storedToken: 'demo-token',
    fetchImpl: async (url) => {
      if (url === '/health') {
  ...
  const html =
    app.elements.platformStatusList.children[0].children[0].children[0].innerHTML;
  assert.match(html, /self_hosted_only/);
});
